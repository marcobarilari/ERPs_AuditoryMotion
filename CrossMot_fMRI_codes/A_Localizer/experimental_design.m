function [names,targets,condition,directions,isTarget] = experimental_design(nrBlocks,numEventsPerBlock,range_targets) 
...
condition= repmat({'static','motion'}, 1,nrBlocks);
...
target_perCondition = randi(range_targets,1,nrBlocks);  
...
%% assign the target events

isTarget=zeros(length(names),numEventsPerBlock);

for block_nr = 1:nr_blocks
    
    numTargetInBlock = targets(block_nr);
    
    % if 1 target pick one from 1 to nr_trials - 1
    if numTargetInBlock==1
        targetsInBlock = datasample([2:numEventsPerBlock-1],numTargetInBlock,'Replace',false);
        isTarget(block_nr,targetsInBlock)=1;
    % if 2 target pick 2 from 1 to nr_trials - 1, and make sure they are
    % at least 1 event apart
    elseif numTargetInBlock==2       
        isConseq = 1;
        while isConseq <=1
            targetsInBlock = datasample([2:numEventsPerBlock-1],numTargetInBlock,'Replace',false);
            isConseq = max(targetsInBlock)-min(targetsInBlock);
        end        
        isTarget(block_nr,targetsInBlock)=1;
    end  
end
